...
test_priority_donate_one (void) 
{
 ...
 // struct list_elem p = thread_current()->elem;
 // msg("%s",&list_entry(p.next,struct thread,elem)->name);
  lock_release (&lock);
  //msg("priority is %d",thread_get_priority());
...
//  msg("In acquire1,lock is held by %s",&lock->holder->name);
  lock_acquire (lock);
//  msg("acquire1:%d",thread_current()->priority);
...
acquire2_thread_func (void *lock_) 
{
  struct lock *lock = lock_;
  lock_acquire (lock);
 //   msg("acquire2:%d",thread_current()->priority);
